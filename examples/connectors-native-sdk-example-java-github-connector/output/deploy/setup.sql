...
GRANT USAGE ON STREAMLIT STREAMLIT.EXAMPLE_JAVA_GITHUB_CONNECTOR_ST TO APPLICATION ROLE DATA_READER;
GRANT USAGE ON STREAMLIT STREAMLIT.EXAMPLE_JAVA_GITHUB_CONNECTOR_ST TO APPLICATION ROLE VIEWER;
...
CREATE OR REPLACE PROCEDURE PUBLIC.GET_REFERENCE_CONFIG(ref_name STRING)
    ...
    AS
        BEGIN
            CASE (ref_name)
                WHEN 'EAI_REFERENCE' THEN
                    RETURN OBJECT_CONSTRUCT(
                        'type', 'CONFIGURATION',
                        'payload', OBJECT_CONSTRUCT(
                            'host_ports', ARRAY_CONSTRUCT('api.github.com'),
                            'allowed_secrets', 'LIST',
                            'secret_references', ARRAY_CONSTRUCT('SECRET_REFERENCE')
                        )
                    )::STRING;
...
CALL PUBLIC.SETUP_EXTERNAL_INTEGRATION_WITH_REFERENCES(ARRAY_CONSTRUCT(
    ...
    'PUBLIC.GITHUB_WORKER(NUMBER, STRING)'));
