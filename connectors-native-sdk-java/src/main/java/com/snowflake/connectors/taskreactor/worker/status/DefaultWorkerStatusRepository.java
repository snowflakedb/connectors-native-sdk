...
class DefaultWorkerStatusRepository implements WorkerStatusRepository {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DefaultWorkerStatusRepository.class);
  ...
  public Set<WorkerId> getAvailableWorkers() {
    logger.debug("Fetching available workers.");
  ...
  public WorkerStatus getStatusFor(WorkerId workerId) {
    logger.debug("Fetching status for worker {}", workerId.value());

    return session
        .table(tableName.getEscapedName())
  ...
  public void updateStatusFor(WorkerId workerId, WorkerStatus status) {
    logger.debug("Updating worker {} status to {}", workerId.value(), status);

    session
        .sql(
            String.format(
                "INSERT INTO %s (WORKER_ID, STATUS) VALUES (%s, '%s')",
                tableName.getEscapedName(), workerId.value(), status.name()))
        .toLocalIterator();
  }

  @Override
  public void updateStatusesFor(WorkerStatus status, List<WorkerId> workerIds) {
    logger.debug("Updating workers {} to status {}.", workerIds, status);

    String values =
        workerIds.stream()
            .map(id -> String.format("(%s, '%s')", id.value(), status.name()))
            .collect(Collectors.joining(","));

    session
        .sql(
            String.format(
                "INSERT INTO %s (WORKER_ID, STATUS) VALUES %s", tableName.getEscapedName(), values))
        .toLocalIterator();
  }

  @Override
  public void removeStatusFor(WorkerId workerId) {
    logger.debug("Removing status of worker {}.", workerId.value());

    session
        .table(tableName.getEscapedName())
        .delete(col(WORKER_ID).equal_to(lit(workerId.value())));
  }
