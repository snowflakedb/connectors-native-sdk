...
public interface CommandsQueue {
  ...
  /**
   * Returns a new instance of the {@link DefaultCommandsQueue} which is default implementation.
   *
   * @param session Snowpark session object
   * @param instanceName task reactor instance schema name
   * @return default, used by the task reactor implementation of the {@link CommandsQueue}
   */
  static DefaultCommandsQueue getInstance(Session session, Identifier instanceName) {
    return new DefaultCommandsQueue(session, instanceName);
  }
