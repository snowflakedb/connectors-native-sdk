...
CREATE OR REPLACE PROCEDURE TASK_REACTOR.REMOVE_INSTANCE(instance_name VARCHAR)
    RETURNS VARIANT
    LANGUAGE SQL
        AS
        $$
            BEGIN
                DROP SCHEMA IF EXISTS identifier(:instance_name);
                DELETE FROM TASK_REACTOR_INSTANCES.INSTANCE_REGISTRY WHERE INSTANCE_NAME = :instance_name;
...
CREATE OR REPLACE PROCEDURE TASK_REACTOR.create_config_table(
    ...
    LANGUAGE SQL
        ...
        BEGIN
            complex_table_name := instance_schema_name || '.' || table_name;
            CREATE TABLE IF NOT EXISTS identifier(:complex_table_name) (KEY VARCHAR, VALUE VARCHAR, UPDATED_AT TIMESTAMP_NTZ DEFAULT SYSDATE());
